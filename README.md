...
این برنامه یه ترکیب از ایده تغییر sni پروتوکل reality و ایده های شخصی هست ؛ که اگر وقت پیدا کنم قراره اپدیت هایی مثل mux هم بهش اضافه کنم
...
```sh
wget  "https://raw.githubusercontent.com/radkesvat/FakeTlsTunnel/install.sh" -O install.sh && chmod +x install.sh && bash install.sh
```
